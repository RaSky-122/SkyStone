...
@Disabled
@Autonomous(name = "Skystones: RED ALLIANCE", group = "skystones")
        ...
        if(opModeIsActive()){
            ...
            switch (position){
                case -1:
                    ...
                    while(Math.abs(new Movement().new Encoders().overallWheelEnc()) <= 800 && !isStopRequested()){
                        motors.sideways(0.5);
                    }
            ...
            motors.front(0.5, 2900);
            motors.stop();
            autoServo1.setPosition(1);

            sleep(300);

            motors.front(-0.5, 750);
            motors.stop();
            while(new Movement().new Encoders().overallWheelEnc() <= 5500 + encoderSkystone && !isStopRequested()) {
                motors.sideways(-0.8);
            }
            ...
            while(new Movement().new Encoders().overallWheelEnc() <= 1850 + encoderSkystone && !isStopRequested()){
                motors.sideways(0.8);
            }
            ...
            while(new Movement().new Encoders().overallWheelEnc() <= encoderSkystone + 800 && !isStopRequested()) {
                motors.sideways(-0.8);
            }
            ...
            while(new Movement().new Encoders().overallWheelEnc() <= 1800 && !isStopRequested()){
                motors.sideways(0.5);
            }
            motors.stop();
    ...
    class Movement{
        ...
        private void sideways(double power) {
            /**if (frontRight.getPower() == 0) {
                ...
                backRight.setPower(-0.02);
                backLeft.setPower(0.02);
            }
            else if (Math.abs(frontLeft.getPower()) <= power) {
                frontRight.setPower(frontRight.getPower() + frontRight.getPower() / 5);
                frontLeft.setPower(-(frontLeft.getPower() + frontLeft.getPower() / 5));
                backRight.setPower(-(backRight.getPower() + backRight.getPower() / 5));
                backLeft.setPower(backLeft.getPower() + backLeft.getPower() / 5);
            }
            else {
